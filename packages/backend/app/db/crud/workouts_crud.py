...
from app.db.crud.exercise_workout_crud import add_exercise_to_workout, update_workout_exercises
...
def create_workout(db: Session, user: AuthUser, title: str, description: str, exercises: list[WorkoutExerciseCreateSchema]):
    ...
    for exercise in exercises:
        add_exercise_to_workout(db, new_workout, exercise)
    db.commit()
...
def create_admin_workout(db: Session):
    ...
    if not admin:
        return
    admin_workout = WorkoutPlan(user_id=admin.id, title="Admin My Workouts", description="This is the default admin workout plan.")
...
def update_workout(db: Session, user: AuthUser, workout_id: UUID, title: str, description: str, exercises: list[WorkoutExerciseCreateSchema]):
    user_id = UUID(user.id)
    workout: WorkoutPlan | None = (db.query(WorkoutPlan).filter(WorkoutPlan.id == workout_id, WorkoutPlan.user_id == user_id).first())
    if workout:
        workout.title = title
        workout.description = description
        update_workout_exercises(db, workout, exercises)
        db.commit()
        db.refresh(workout)
