...
async def create_workout(new_workout: WorkoutPlanCreateSchema, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    create_workout_db(db, user, new_workout.name, new_workout.description)
    return {"success": True}

@router.delete("/delete/{workout_id}", status_code=204)
async def delete_workout(workout_id: UUID, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
